		...
		"indexedDB": function(dbName, config) {
			...
			var wrap = {
					...
					return $.Deferred(function(dfd) {
						try {
							...
							idbRequest.onsuccess = function(e) {
								if(debug) console.log("Success", idbRequest, e, this);
								dfd.resolveWith(idbRequest, [idbRequest.result, e]);
							};
							idbRequest.onerror = function(e) {
								if(debug) console.log("Error", idbRequest, e, this);
							...
							if (typeof idbRequest.onblocked !== "undefined" && idbRequest.onblocked === null) {
								idbRequest.onblocked = function(e) {
									if(debug) console.log("Blocked", idbRequest, e, this);
									var res;
									try {
							...
							if (typeof idbRequest.onupgradeneeded !== "undefined" && idbRequest.onupgradeneeded === null) {
								idbRequest.onupgradeneeded = function(e) {
									if(debug) console.log("Upgrade", idbRequest, e, this);
				...
				"cursor": function(idbCursor, callback) {
					return $.Deferred(function(dfd) {
						try {
							if(debug) console.log("Cursor request created", idbCursor);
							var cursorReq = typeof idbCursor === "function" ? idbCursor() : idbCursor;
							cursorReq.onsuccess = function(e) {
								if(debug) console.log("Cursor successful");
								if (!cursorReq.result) {
								...
								if(debug) console.log("Cursor in progress", elem, e);
								dfd.notifyWith(cursorReq, [elem, e]);
								var result = callback.apply(cursorReq, [elem]);
								if(debug) console.log("Iteration function returned", result);
								try {
									if (result === false) {
										dfd.resolveWith(cursorReq, [null, e]);
									...
									if(debug) console.log("Exception when trying to advance cursor", cursorReq, e);
									dfd.rejectWith(cursorReq, [cursorReq.result, e]);
								}
							};
							cursorReq.onerror = function(e) {
								if(debug) console.log("Cursor request errored out", e);
								dfd.rejectWith(cursorReq, [cursorReq.result, e]);
							};
						} catch (e) {
							if(debug) console.log("An exception occured inside cursor", cursorReq, e);
							e.type = "exception";
				...
				"index": function(index) {
					...
					if(debug) console.log(idbIndex, index);
					return {
			...
			var dbPromise = wrap.request(function() {
				if(debug) console.log("尝试打开数据库版本：", version);
				return version ? indexedDB.open(dbName, parseInt(version)) : indexedDB.open(dbName);
			});
			dbPromise.then(function(db, e) {
				if(debug) console.log("DB opened at", db.version);
				...
				if (e && e.type === "upgradeneeded") {
					if (config && config.schema) {
						...
						if(debug) console.log("Upgrading DB to ", db.version);
					...
					return $.Deferred(function(dfd) {
						dbPromise.then(function(db, e) {
							...
							try {
								...
								if(debug) console.log("Creating a traction failed", e, storeNames, mode, this);
								e.type = "exception";
							...
							if(debug) console.log("Database open is blocked or upgrade needed", res, e.type);
					...
					function op(callback) {
						return $.Deferred(function(dfd) {
							function onTransactionProgress(trans, callback) {
								try {
									if(debug) console.log("Finally, returning the object store", trans);
									...
									if(debug) console.log("Duh, an exception occured", e);
									e.name = "exception";
									dfd.rejectWith(trans, [e, e]);
								}
							}
							me.transaction(storeName, getDefaultTransaction(mode)).then(function() {
								if(debug) console.log("Transaction completed");
								...
								if (err.code === err.NOT_FOUND_ERR && (mode === true || typeof mode === "object")) {
									if(debug) console.log("Object Not found, so will try to create one now");
									var db = this.result;
									db.close();
									dbPromise = wrap.request(function() {
										if(debug) console.log("Now trying to open the database again", db.version);
										return indexedDB.open(dbName, (parseInt(db.version, 10) || 1) + 1);
									});
									dbPromise.then(function(db, e) {
										if(debug) console.log("Database opened, tto open transaction", db.version);
										db.onversionchange = function() {
											// Try to automatically close the database if there is a version change request
											if (!(config && config.onversionchange && config.onversionchange() !== false)) {
												db.close();
										...
										me.transaction(storeName, getDefaultTransaction(mode)).then(function() {
											if(debug) console.log("Transaction completed when trying to create object store");
											// Nothing much to do
										}, function(err, e) {
											dfd.rejectWith(this, [err, e]);
										}, function(trans, e) {
											if(debug) console.log("Transaction in progress, when object store was not found", this, trans, e);
										...
										if (e.type === "upgradeneeded") {
											try {
												if(debug) console.log("Now trying to create an object store", e.type);
												db.createObjectStore(storeName, mode === true ? {
													"autoIncrement": true
												} : mode);
												if(debug) console.log("Object store created", storeName, db);
											} catch (ex) {
												if(debug) console.log("Exception when trying ot create a new object store", ex);
								...
								if(debug) console.log("Transaction is in progress", trans);
								onTransactionProgress(trans, callback);
							});
						});
					}
